// KOVA Menu - MOBILE LOCKED - NO SIDE DRAG - HEART ON CARDS ONLY - STICKY PILL BAR DOCKED TO NAV + D1 GUEST AUTH SAFE + FLAME VIEW + FLOATING CHECKOUT FIXED
...
export const Menu = {
  ...
  openImgModal(src, title=''){
    ...
    if(!m||!img) return;
    img.src=src; if(t) t.textContent=title||'';
...
[data-theme="light"] #cartBar span { color: #F4F4F5!important; }
[data-theme="light"].cart-bar button,
[data-theme="light"] #cartBar button { color: #000!important; }
[data-theme="light"].cart-bar button:last-child,
[data-theme="light"] #cartBar button:last-child {
  background: #F4F4F5!important;
  color: #0A0A0B!important;
}
.img-modal{position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.82);backdrop-filter:blur(18px);display:none;place-items:center;padding:16px}
.img-modal.active{display:grid}
.img-modal-box{position:relative;max-width:90vw;max-height:90vh}
.img-modal-box img{max-width:90vw;max-height:80vh;border-radius:18px;object-fit:cover;display:block;box-shadow:0 20px 60px rgba(0,0,0,.6)}
      </style>
      ...
      <div class="menu-wrap" id="menuWrap">
        <div class="menu-head">
          <div style="min-width:0">
            ...
            <p style="color:var(--text-muted);font-size:11px;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Filter: ${this.activeFilter} • ${token?'☁️ D1 Synced':'📱 Local'} • Tap image for flame view</p>
          </div>
        ...
        <div id="cartBar" style="position:fixed;bottom:16px;left:50%;transform:translateX(-50%);z-index:99999;display:${cartCount?'flex':'none'};gap:8px;align-items:center;background:#F4F4F5;border-radius:999px;padding:6px 8px 6px 12px;box-shadow:0 12px 32px rgba(0,0,0,.35);border:1px solid #E5E5E5;white-space:nowrap;height:42px">
          ...
          <button id="floatCheckoutBtn" style="background:#E6E82A;color:#000;border:0;padding:8px 14px;border-radius:999px;font-weight:900;font-size:11px;cursor:pointer;white-space:nowrap">Checkout</button>
          <button id="floatOrdersBtn" style="background:transparent;color:#000;border:0;padding:8px 10px;font-weight:700;font-size:11px;cursor:pointer;white-space:nowrap">My Orders</button>
        </div>
  ...
  async afterRender() {
    ...
    const loadTrending = async ()=>{
      if(!token) return;
      try{
        const r=await fetch(`${GUEST_WORKER}/api/guest/orders`,{headers:{'Authorization':`Bearer ${token}`}});
        const d=await r.json();
        const orders=d.orders||[];
        const map={};
        orders.forEach(o=>{
          let items=[]; try{ items=typeof o.items==='string'?JSON.parse(o.items):o.items }catch{}
          items.forEach(it=>{ const id=String(it.id||it.item_id); map[id]=(map[id]||0)+(it.qty||1); });
        });
        this.popular=map;
        localStorage.setItem('kova_popular', JSON.stringify(map));
      }catch{
        try{ this.popular=JSON.parse(localStorage.getItem('kova_popular')||'{}'); }catch{ this.popular={}; }
      }
    };
    loadTrending();
    ...
    const token = localStorage.getItem('kova_token');
       ...
       // RANDOMIZE ALL ON EVERY RELOAD
    if((this.activeFilter||'All').toLowerCase()==='all'){
      for(let i=this.dishes.length-1;i>0;i--){
        const j=Math.floor(Math.random()*(i+1));
        [this.dishes[i],this.dishes[j]]=[this.dishes[j],this.dishes[i]];
      }
    }
    ...
    const updateTopbarCart = () => {
      ...
      if(this.activeFilter!=='All'){
        ...
        if(f==='trending'){
          // Sort by most ordered count
          const pop=this.popular||{};
          data.sort((a,b)=> (pop[String(b.id)]||0) - (pop[String(a.id)]||0) );
          // If no orders yet, fallback to badge trending
          const hasOrders=Object.keys(pop).length>0;
          if(!hasOrders) return data.filter(d=> (`${d.badge} ${d.tags.join(' ')}`.toLowerCase().includes('trend')) ).length? data.filter(d=> (`${d.badge} ${d.tags.join(' ')}`.toLowerCase().includes('trend')) ) : data;
          return data;
        }
        data = data.filter(d=>{
      ...
      for(let i=0;i<12;i++){
        ...
        grid.insertAdjacentHTML('beforeend', `
          <div class="k-card" data-id="${d.id}">
            <div onclick="Menu.openImgModal('${d.image}','${(d.title||'').replace(/'/g,"&#39;")}')" style="height:180px;background:${d.image?`url(${d.image}) center/cover`:`linear-gradient(135deg,#1C1C1F,#27272A)`};position:relative;overflow:hidden;cursor:zoom-in">
    ...
    // Floating cart bar wiring - FIXED
    const floatCheckout = document.getElementById('floatCheckoutBtn');
    const floatOrders = document.getElementById('floatOrdersBtn');

    if(floatCheckout){
      floatCheckout.onclick = async ()=>{
        if(this.cart.length===0) return;
        // Go to orders and trigger checkout modal
        localStorage.setItem('kova_pending_checkout','1');
        location.hash='#/orders';
      };
    }
    if(floatOrders){
      floatOrders.onclick = ()=>{ location.hash='#/orders'; };
    }
